...
public class EntityControllerTest {
    ...
    EntityReferenceService referenceService;
    //Class under test
    EntityController controller;

    @BeforeAll
    void setUp(){
        referenceService = new EntityReferenceService(entityDao);
    ...
    void testPatchNewSingleEntity(){
        ...
        assertTrue(response.getBody().entityAttributes().size() == 2);
        assertEquals(new EntityAttributes(Map.of("created_at", "2022-10-01", "foo", "bar")), response.getBody().entityAttributes());
    }
    @Test
    void testPatchSingleEntityOverwriteAttr(){
        ...
        assertTrue(response.getBody().entityAttributes().size() == 2);
        assertEquals(new EntityAttributes(Map.of("created_at", "2022-10-01", "foo", "baz")), response.getBody().entityAttributes());
    }
    @Test
    void testPatchNewSingleEntityWithReference(){
        ...
        assertTrue(response.getBody().entityAttributes().size() == 3);
        assertEquals(new EntityAttributes(Map.of("created_at", "2022-10-01", "foo", "bar", "referencingAttr", refAttr)), response.getBody().entityAttributes());
    }
    ...
    void testGetSingleEntity(){
        Entity testEntity = new Entity(testEntityId,
                testEntityType, new EntityAttributes(Map.of("created_at", "2022-10-01", "foo", "bar")), 1L);
        when(entityDao.getSingleEntity(any(), any(), any())).thenReturn(testEntity);
