...
start() {
    ...
    BASEDIR=$(dirname "$0")
    docker create --name $CONTAINER --rm -e POSTGRES_PASSWORD=password -e PGHOST=5434  -p "$POSTGRES_PORT:5432" postgres:$POSTGRES_VERSION -c listen_addresses='*'
...
CONTAINER=postgresDestination
COMMAND=$1
POSTGRES_PORT=${2:-"5434"}
