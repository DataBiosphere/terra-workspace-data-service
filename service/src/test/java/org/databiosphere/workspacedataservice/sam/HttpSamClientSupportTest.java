...
class HttpSamClientSupportTest {
  ...
  @DisplayName(
      "When Sam throws an ApiException with standard http status code 401, HttpSamClientSupport should throw AuthenticationException")
  @Test
  void authenticationException() {
    ...
    assertThrows(
        AuthenticationException.class,
        () ->
            httpSamClientSupport.withRetryAndErrorHandling(
                voidSamFunction, "AuthenticationException"),
        "voidSamFunction should throw AuthenticationException");
  }

  @DisplayName(
      "When Sam throws an ApiException with standard http status code 403, HttpSamClientSupport should throw AuthorizationException")
  @Test
  void authorizationException() {
    ...
    assertThrows(
        ...
        () ->
            httpSamClientSupport.withRetryAndErrorHandling(
                voidSamFunction, "AuthorizationException"),
        "voidSamFunction should throw AuthorizationException");
  }

  @DisplayName(
      "When Sam throws a NullPointerException, HttpSamClientSupport should throw SamException(500)")
  @Test
  ...
  @DisplayName(
      "When Sam throws a RuntimeException, HttpSamClientSupport should throw SamException(500)")
  @Test
  ...
  @ParameterizedTest(
      name =
          "When Sam throws an ApiException with nonstandard http status code {0}, HttpSamClientSupport should throw SamException with code 500")
  ...
  @ParameterizedTest(
      name =
          "When Sam throws an ApiException with standard http status code {0}, HttpSamClientSupport should throw SamException with the same code")
  ...
  void retryableExceptions(int samCode) {
    ...
    assertThrows(
        Exception.class,
        () -> httpSamClientSupport.withRetryAndErrorHandling(samFunction, "retryableExceptions"));
    ...
    assertThrows(
        Exception.class,
        () ->
            httpSamClientSupport.withRetryAndErrorHandling(voidSamFunction, "retryableExceptions"));
    // with current settings, will retry 5 times. Any retry means we'll have more than
    // one invocation.
  ...
  private void expectSamExceptionWithStatusCode(
      int expectedStatusCode, SamFunction<?> samFunction) {
    SamException actual =
        assertThrows(
            SamException.class,
            () ->
    ...
    assertEquals(
        ...
        "samFunction: Incorrect status code in SamException");
  }

  private void expectSamExceptionWithStatusCode(
      int expectedStatusCode, VoidSamFunction voidSamFunction) {
    SamException actual =
        assertThrows(
            SamException.class,
            () ->
                httpSamClientSupport.withRetryAndErrorHandling(
                    voidSamFunction, "samFunction-unittest"),
            "voidSamFunction should throw SamException");

    assertEquals(
        expectedStatusCode,
        actual.getRawStatusCode(),
        "voidSamFunction: Incorrect status code in SamException");
  }
}
