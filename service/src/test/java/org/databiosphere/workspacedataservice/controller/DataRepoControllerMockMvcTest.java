...
class DataRepoControllerMockMvcTest {
    ...
    void importSnapshotWithPermission() throws Exception {
        ...
        given(mockRepositoryApi.retrieveSnapshot(any(), any()))
                .willReturn(new SnapshotModel().id(uuid).name("foo").tables(List.of(new TableModel().name("table1"), new TableModel().name("table2"))));
    ...
    void importSnapshotWithoutPermission() throws Exception {
        ...
        UUID uuid = UUID.randomUUID();
        mockMvc.perform(post("/{instanceId}/snapshots/{version}/{snapshotId}", instanceId, versionId, uuid)).andExpect(status().isForbidden());
    }
    ...
    void importSnapshotWithPolicyConflict() throws Exception {
        ...
        mockMvc.perform(post("/{instanceId}/snapshots/{version}/{snapshotId}", instanceId, versionId, uuid)).andExpect(status().isConflict());
    }
}
