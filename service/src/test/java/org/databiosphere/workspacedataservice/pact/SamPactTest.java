...
class SamPactTest {
  ...
  private SamAuthorizationDao getSamAuthorizationDao(
    ...
    return getSamDaoFactory(mockServer).getSamAuthorizationDao(workspaceId);
  }

  private SamDao getSamDao(MockServer mockServer) {
    return getSamDaoFactory(mockServer).getSamDao();
  }

  private SamDaoFactory getSamDaoFactory(MockServer mockServer) {
    return new SamDaoFactory(new HttpSamClientFactory(mockServer.getUrl()), new RestClientRetry());
  }
