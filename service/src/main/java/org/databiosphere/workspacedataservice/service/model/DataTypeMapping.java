...
public enum DataTypeMapping {
	NULL(null, "text", false, "?"), EMPTY_ARRAY(String[].class, "text[]", true, "?"), BOOLEAN(null, "boolean", false,
			"?"), DATE(null, "date", false, "?"), DATE_TIME(null, "timestamp with time zone", false, "?"), STRING(null,
					"text", false,
					"?"), JSON(null, "jsonb", false, "?::jsonb"), NUMBER(null, "numeric", false, "?"), ARRAY_OF_NUMBER(
							Double[].class, "numeric[]", true, "?::numeric[]"), ARRAY_OF_DATE(String[].class, "date[]",
									true, "?::date[]"), ARRAY_OF_DATE_TIME(String[].class, "timestamp with time zone[]",
											true, "?::timestamp with time zone[]"), ARRAY_OF_STRING(String[].class,
													"text[]", true,
	...
	static {
		Arrays.stream(DataTypeMapping.values()).filter(v -> !EnumSet.of(EMPTY_ARRAY, NULL).contains(v))
