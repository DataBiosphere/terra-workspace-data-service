...
/**
 * Servlet request listener that sets a unique request id into the SLF4J MDC
 * context for this request. This listener does this by looking in the incoming
 * request headers for an "x-b3-traceid", "x-request-id", or "trace-id" value
 * (in that order) and using the first 64 characters of that value if present.
 * <p>
 * If no id is present in the incoming request headers, it will assign a new
 * UUID value for the request id.
 * <p>
 * See also {@link MDCResponseHeaderFilter}, which adds the "requestId" value to
 * the MDC context.
 *
 */
@Component
public class MDCServletRequestListener implements ServletRequestListener {
	...
	// the list of request headers, in order, we will search for an incoming unique
	// request id
	// we could add other headers here, such as: x-vcap-request-id,
	// X─B3─ParentSpanId, X─B3─SpanId, X─B3─Sampled, x-amzn-trace-id
