...
public class DataRepoService {
  ...
  public void importSnapshot(UUID instanceId, UUID snapshotId) {
    ...
    // policy conflicts
    workspaceManagerDao.createDataRepoSnapshotReference(snapshot, false);
  ...
  public void addSnapshot(SnapshotModel snapshot, UUID instanceId) {
    ...
    Map<String, DataTypeMapping> schema =
        Map.of(
            TDRIMPORT_SNAPSHOT_ID,
            DataTypeMapping.STRING,
            TDRIMPORT_IMPORT_TIME,
    ...
    if (!recordDao.recordTypeExists(instanceId, importType)) {
      ...
      activityLogger.saveEventForCurrentUser(
          user -> user.created().table().withId(snapshot.getName()));
    }
      ...
      records.add(
          new Record(
              ...
              RecordAttributes.empty()
                  .putAttribute(TDRIMPORT_SNAPSHOT_ID, snapshot.getId())
                  .putAttribute(TDRIMPORT_IMPORT_TIME, LocalDateTime.now())));
    }
    recordDao.batchUpsert(instanceId, importType, records, schema);
    if (!records.isEmpty()) {
      activityLogger.saveEventForCurrentUser(
          user -> user.created().record().withRecordType(importType).ofQuantity(records.size()));
    }
  }
}
