...
public class PostgresRestoreDao extends AbstractBackupRestoreDao<RestoreResponse> {
  ...
  public Job<JobInput, RestoreResponse> getStatus(UUID trackingId) {
    ...
    if (responses.size() == 1) {
      ...
      throw new RuntimeException(
          "Unexpected error: %s rows found for restore status query".formatted(responses.size()));
    }
  }
  ...
  public void createEntry(UUID trackingId, BackupRestoreRequest BackupRestoreRequest) {
    ...
    namedTemplate
        .getJdbcTemplate()
        .update(
            "insert into sys_wds.restore(id, status, createdtime, updatedtime, requester, description) "
                + "values (?,?,?,?,?,?)",
            trackingId,
  ...
  private static class RestoreJobRowMapper implements RowMapper<Job<JobInput, RestoreResponse>> {
    @Override
    public Job<JobInput, RestoreResponse> mapRow(ResultSet rs, int rowNum) throws SQLException {
      ...
      try {
        ...
        LOGGER.warn(
            "Unknown status for restore job {}: [{}] with error {}",
            jobId,
            dbStatus,
            e.getMessage());
      }
