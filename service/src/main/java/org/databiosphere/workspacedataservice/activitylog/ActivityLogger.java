...
/**
 * Save entries to an activity log. Currently implemented as writing to a Slf4j logger, but could be
 * modified to write entries to a database table or other persistence.
 */
public class ActivityLogger {
  ...
  /**
   * creates a new, empty event. In most cases, callers should use #saveEventForCurrentUser instead.
   *
  ...
  /**
   * Initializes a new event for the current user, allows the caller to specify the action, target,
   * ids, etc. for that event, then saves that event. Example:
