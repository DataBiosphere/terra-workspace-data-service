...
/**
 * Utility/support functions for HttpSamClient: - logging of all requests to Sam - exception
 * handling for Sam errors: - if we catch an ApiException with 401 http status from the Sam client,
 * throw AuthenticationException - if we catch an ApiException with 403 http status from the Sam
 * client, throw AuthorizationException - if we catch an ApiException with other well-known http
 * status from the Sam client, throw SamException with the same status - if we catch an ApiException
 * with non-parseable http status from the Sam client, throw SamException with status 500 - if we
 * catch a non-ApiException from the Sam client, throw SamException with status 500 - retry calls to
 * Sam on retryable exceptions
 */
public class HttpSamClientSupport {
  ...
  /**
   * Perform a Sam client request with logging and exception handling, and return the result of that
   * request.
   *
   ...
   */
  @Retryable(
      include = {SamRetryableException.class},
      maxAttemptsExpression = "${sam.retry.maxAttempts}",
      backoff =
          @Backoff(
              delayExpression = "${sam.retry.backoff.delay}",
              multiplierExpression = "${sam.retry.backoff.multiplier}"),
      listeners = {"retryLoggingListener"})
  public <T> T withRetryAndErrorHandling(SamFunction<T> samFunction, String loggerHint)
      throws SamException, AuthenticationException, AuthorizationException {
    try {
      ...
      LOGGER.error(
          loggerHint + " Sam request resulted in ApiException(" + apiException.getCode() + ")",
          apiException);
  ...
  @Retryable(
      ...
      backoff =
          @Backoff(
              delayExpression = "${sam.retry.backoff.delay}",
              multiplierExpression = "${sam.retry.backoff.multiplier}"))
  public void withRetryAndErrorHandling(VoidSamFunction voidSamFunction, String loggerHint)
      throws SamException, AuthenticationException, AuthorizationException {
